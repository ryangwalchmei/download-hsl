...
export async function downloadHSL(url, outputFilename, res) {
  return new Promise((resolve, reject) => {
    ...
    if (!fs || !ffmpeg) {
      reject(
        new Error("Este código só pode ser executado no lado do servidor"),
      );
      return;
    }
    ...
    const ffmpegStream = ffmpeg(url) // Passa diretamente a URL de entrada
      .outputOptions([
        // Defina as opções de saída
        "-c:v",
        "copy", // Copia o codec de vídeo
        "-c:a",
        "copy", // Copia o codec de áudio
        "-bsf:a",
        "aac_adtstoasc", // Ajusta o áudio para AAC
        "-protocol_whitelist",
        "file,http,https,tls,crypto", // Permite protocolos necessários
      ])
      .format("mp4")
